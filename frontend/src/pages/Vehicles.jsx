...
const EMPTY_FORM = {
  registrationNumber: '', name: '', type: 'Van', maxLoadCapacity: '', odometer: '', acquisitionCost: '', region: '', status: 'Available',
};
...
export default function Vehicles() {
  ...
  const load = () => {
    ...
    api.get('/vehicles', { params }).then(({ data }) => setVehicles(data)).finally(() => setLoading(false));
  };
  ...
  const openEdit = (v) => {
    setEditing(v);
    setForm({
      registrationNumber: v.registrationNumber, name: v.name, type: v.type,
      maxLoadCapacity: v.maxLoadCapacity, odometer: v.odometer, acquisitionCost: v.acquisitionCost,
  ...
  return (
    <div>
      <div className="mb-6 flex items-end justify-between">
        <div>
          <p className="mono text-[10px] uppercase tracking-widest text-base-400">Vehicle Registry</p>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={8} className="px-4 py-6 text-center text-base-400">No vehicles registered yet.</td></tr>
            ) : vehicles.map((v) => (
              <tr key={v._id} className="border-b border-base-800 last:border-0 hover:bg-base-800/50">
                <td className="mono px-4 py-3">{v.registrationNumber}</td>
                ...
                <td className="px-4 py-3"><StatusTag status={v.status} /></td>
                {canManage && (
                  <td className="px-4 py-3">
                    <div className="flex justify-end gap-2">
                      <button onClick={() => openEdit(v)} className="text-base-400 hover:text-signal-500"><Pencil className="h-4 w-4" /></button>
                      <button onClick={() => remove(v)} className="text-base-400 hover:text-danger-500"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                )}
              </tr>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit vehicle' : 'Register vehicle'}>
        ...
        <form onSubmit={submit}>
          <Field label="Registration number" required>
            <Input value={form.registrationNumber} onChange={(e) => setForm({ ...form, registrationNumber: e.target.value })} required placeholder="DL-01-AB-1234" />
          </Field>
          <Field label="Name / model" required>
            <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required placeholder="Tata Ace Gold" />
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Type" required>
              <Select value={form.type} onChange={(e) => setForm({ ...form, type: e.target.value })}>
                {(meta.types.length ? meta.types : ['Van', 'Truck', 'Mini Truck', 'Trailer', 'Bike']).map((t) => (
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <Field label="Acquisition cost" required>
              <Input type="number" min="0" value={form.acquisitionCost} onChange={(e) => setForm({ ...form, acquisitionCost: e.target.value })} required />
            </Field>
          </div>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Region">
              <Input value={form.region} onChange={(e) => setForm({ ...form, region: e.target.value })} placeholder="North" />
            </Field>
            {editing && (
              <Field label="Status">
          ...
          <div className="mt-4 flex justify-end gap-2">
            ...
            <PrimaryButton type="submit" disabled={saving}>{saving ? 'Saving…' : 'Save vehicle'}</PrimaryButton>
          </div>
        </form>
      </Modal>
    </div>
